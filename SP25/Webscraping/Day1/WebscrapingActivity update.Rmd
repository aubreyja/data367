...
team_html <- html_nodes(site_info, '#stat_grid td:nth-child(1)')
...
#goals_html <- html_nodes(site_info, '#stat_grid .left:nth-child(3)')
...
```{r}
assists_html <- html_nodes(site_info, '.text td:nth-child(4)')
assists <- html_text(assists_html)
assists <- as.numeric(trimws(assists, 'both'))
head(assists)
```
...
soccer.df <- data.frame(team, conf, goals, assists)
head(soccer.df)
```
...
```{r}
plot(goals ~ assists, data = soccer.df)
